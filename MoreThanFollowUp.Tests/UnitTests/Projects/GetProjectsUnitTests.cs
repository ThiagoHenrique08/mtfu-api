...
namespace MoreThanFollowUp.Tests.UnitTests.Projects
{
    public class GetProjectsUnitTests 
    {
        ...
        public async Task GetProjectsPagination_ShouldReturnOkWithPaginationMetadata()
        {
            ...
            var parameters = new ProjectsParameters { PageNumber = 1, PageSize = 2 };

            _mockRepo.Setup(repo => repo.GetProjectPaginationAsync(It.IsAny<ProjectsParameters>())).ReturnsAsync(new StaticPagedList<Project>(
            new List<Project>
            {
            ...
            // Act
            var result = await controller.GetProjectsPagination(parameters);
